...
public class ProductForm extends JPanel {
    ...
    private void showDialog(Product prod) {
        ...
        JComboBox<String> cb = new JComboBox<>(new String[] { "Dog", "Cat", "Accessory" });
        if (edit)
            cb.setSelectedItem(prod.getType());
        cb.setFont(new Font("Helvetica Neue", Font.PLAIN, 14));
        JTextField tfPrice = mkFld(edit ? String.valueOf((int) prod.getPrice()) : "");

        form.add(mkLbl("Tên sản phẩm"));
        form.add(tfName);
        form.add(mkLbl("Danh mục"));
        form.add(cb);
        form.add(mkLbl("Giá (đồng)"));
        form.add(tfPrice);
